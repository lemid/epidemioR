...
```{r, image11, echo = FALSE, fig11 = '(ref:fig11)'}
...
```{r grafico1, echo = FALSE, grafico1 = '(ref:grafico1)'}
### Importação dos dados em .csv salvos no github
...
grafico <- ggplot(tb, aes(x = conc, y = inib)) +
  ...
  labs(x = "Concentração",
       y = "Inibição (%)")
```
(ref:grafico1) Exemplo de curvas de EC50. 
...
ggplot(tb,
       aes(x = conc, y = inib)) +
  facet_wrap(facets = ~teste*prod) +
  geom_point()
```

```{r}
# Ajustando um modelo log-logístico de três parâmetros
drc1.LL3 <- drm(inib ~ conc, teste:prod, data = tb, fct = LL.3())
```
...
mselect(drm(inib ~ conc, teste:prod, data = tb, fct = LL.3()))

# Montagem de um modelo com nomes atribuídos aos parâmetros
drc2.LL.3 <- drm(inib ~ conc, teste:prod, data = tb, 
...
params <- matrix(coef(drc2.LL.3),
                 ncol = 3,
                 dimnames = list(levels(tb$teste:tb$prod),
                                 c("C", "A", "B")))
...
EC50[] <- lapply(EC50, gsub, pattern='e:', replacement='') 
EC50[] <- lapply(EC50, gsub, pattern=':50', replacement='') 
EC50
