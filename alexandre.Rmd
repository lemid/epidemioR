...
```
(ref:fig1) Escala diagramática da ferrugem da soja [@godoy2005].
...
tb_params <- params%>%as.data.frame()%>%
  mutate(EC50 = log(-(log(50/A))/B)/log(C))
...
Os dados de sensibilidade de *P. pachyrhizi* podem ser analisados pelo pacote DRC [@Ritz2015]. Para tanto, são necessários alguns pacotes que precisam ser carregados antes de iniciar o script. O carregamento dos pacotes é realizado pelos comandos abaixo.
...
### Comparação entre métodos de estimar o EC50

Um método muito utilizado para estimar os valores de EC50 é o uso da linearização pelo log da concentração. Essa prática frequentemente utiliza o software excel, entretanto, podemos realizar a estimativa pelo R.

```{r}
### Importação dos dados em .csv salvos no github
tb <- read.csv2("https://raw.githubusercontent.com/lemid/epidemioR/master/alexandre/log.csv", sep=";", dec=",", header = T)
...
```{r}
# Aplicação do modelo
lin1 <- lm(inib ~ conc*trat, tb)
summary(lin1)

# Resíduos e valores ajustados para inspecionar.
tb$resL1 <- residuals(lin1)
tb$fitL1 <- fitted(lin1)

### Predição dos valores de EC50
modelo <- lm(data = tb, inib ~ conc* trat)

```

```{r}
library(ggplot2)
ggplot(tb, aes(x = conc, y = inib)) +
  facet_wrap(facets = ~trat) +
  geom_point() +
  geom_line(data = tb,
            mapping = aes(x = conc, y = fitL1))
```
```{r}
coef(lin1)
```
...
```{r, image11, fig11 = '(ref:fig11)'}
knitr::include_graphics("./Alexandre/fig11.jpg")
```
(ref:fig11) Exemplo de um ensaio de EC50 com nematóides. Gradientes de eclosão resultantes de 5 concentrações de nematicida em um experimento com quatro repetições.
...
Os valores de inibição foram obtidos pela equação de SCHNEIDER-ORELLI, 1947 foram expressos na planilha na coluna "inib". Após a correção pela equação, a mortalidade natural da testemunha é eliminada dos dados, deste modo, os valores da testemunha após a correção são valores próximos de 0 (zero).

A importação dos dados da planilha será realizada usando os comandos abaixo:

```{r}
### Importação dos dados em .csv salvos no github
tb <- read.csv2("https://raw.githubusercontent.com/lemid/epidemioR/master/alexandre/meloidogyne.csv", 
                sep=";", dec=",", header = T)
...
```{r}
# Ajustando um modelo log-logístico de três parâmetros
drc1.LL3 <- drm(inib ~ conc, trat, data = tb, fct = LL.3())
```

```{r}
# Selecionando o melhor modelo global
mselect(drc1.ll3, list(L.3(), LN.3(), W2.3(), G.3()))
```

```{r}
# Calculando o ajuste do modelo
mselect(drm(inib ~ conc, trat, data = tb, fct = LL.3()))

# Montagem de um modelo com nomes atribuídos aos parâmetros
drc2.LL.3 <- drm(inib ~ conc, trat, data = tb, 
...
```{r}
pred <- predict(drc2.LL.3, interval = "confidence")
```
...
```{r}
# valor de Ec50, Erro e intervalo de confiança
EC50 <- ED(drc2.LL.3, 50, interval = "delta")
