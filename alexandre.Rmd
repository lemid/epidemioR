...
Para a *P. pachyrhizi* os estudos de CE~50~ - concentração efetiva para controlar 50% do patógeno são realizados estudos em folha destacada (*ex vivo*) em placas de Petri, pois permite avaliar a sensibilidade do fungo em condições controladas de laboratório, sem demandar de grande estrutura. 
...
Após a determinação dos pontos de fixação da curva pelo e determinação dos resíduos, é recomendável predizer o maior número de pontos na curva modelada. Essa predição é recomendável para aumentar o ajuste da curva. Utilizaremos em nosso exemplo, uma curva predita para cada 0,5 unidade do eixo "X". Para cada valor de "X" serão calculados todos os parâmetros da curva.
...
```
(ref:fig11) A esquerda uma fêmea adulta de *Meloidogyne javanica* e a direita um adulto de *helicotylenchus dihystera*. Ambos são nematoides fitoparatitas e apresentam estilete para se alimentar sugando o conteudo celular.
...
tb_params <- params%>%as.data.frame()%>% 
  mutate(CE50 = exp((log(((A/(100/2))-1))/(C))+B))
```

Este exemplo de CE~50~ com *Meloidogyne* é semelhante ao exemplo com *Phakopsora pachyrhizi*, mas a maior diferença surge nesse ponto do script. Algumas microrganismos ou isolados, não são afetadados pela testemunha, e são afetados ao máximo pela concentração mais alta utilizada em um teste de CE~50~. Em situações assim, a curva inicia-se no zero, apresenta um crescimento ou queda acentuada, para então estabilizar na máxima concentração. Entretanto, alguns microrganismos não apresentam um platô de estabilidade da curva, exigindo tratamentos com concentrações extremamente altas. Situações assim necessitam de ajustes no script.

No caso dos nematoides, temos conciência que a resposta ocorre entre 0 e 100%, ou seja, há possibilidade de que todos os nematoides estejam mortos ou vivos, ou que todos os juvenis eclodam ou não. Assim, podemos conciderar a curva com a amplitude máxima de 100%. No caso das ferrugens, a amplitude da curva não pode ser analisada da mesma forma. Vamos usar como exemplo a ferrugem do pessegueiro. As folhas de pessegueiro suportam que a ferrugem causada por *Tranzschelia discolor* atinja uma severidade máxima de 30% antes de sofrer abscissão [@DOLINSKI2017]. Essa situação,demonstra que a assintota máxima da curva será limitada ou próxima da severidade máxima de 30%, respectivamente. Deste modo, a CE~50~ será estimada atingir um nível máximo de severidade da doença de 15%. Vários patossistemas apresentam esse comportamento limitante, como o mildio da videira (*Plasmopara viticola*) que apresenta severidade máxima de 72% [@CAMARGO2019], a ferrugem alaranjada da cana-de-açucar (*Puccinia kuehnii*) que apresenta severidade máxima de 45% [@KLOSOWSKI2013] e a ferrugem da ameixeira (*Tranzschelia discolor*) com severidade máxima de 35% [@VIDAL2019].
...
Ao considerarmos que a utilização correta de ambas as ferramentas garantem o acerto, cada pesquisadorpoderá optar pela ferramenta que mais se adapta as suas necessidades. Destacamos que o pacote 'drc' possui diversas facilidades devido a suas funções próprias do pacote, que facilitam a execução e reduzem o script. Entretanto, o 'nlme' permitiria realizar análises com modelos não lineares que não estão programados do 'drc'. Este capítulo tem a função de facilitar o acesso a essas funcionalidades, facilitando o dia a dia da pesquisa com fitopatologia e nematologia.
